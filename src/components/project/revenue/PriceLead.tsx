...
const PriceZinc: React.FC<PriceZincProps> = ({
    ...
    const [Price_L_Price, setPrice_L_Price] = useState<string>('London Metal Exchange');
    ...
    useMemo(() => {
        ...
        if (formFieldsState.length > 0) {
            ...
            formFieldsState.forEach((fieldState) => {
                if (fieldState.key === 'Price_L_Price') {
                    if (valList.indexOf(fieldState.key) >= 0) {
                        setPrice_L_Price(fieldState.value);
                    }
                }
            }
            );
    ...
    const handleInputChange = (key: string, value: string) => {
        ...
        if (key === 'Price_L_Price') {
            setPrice_L_Price(value);
        }
                ...
                <div className="flex justify-start  mt-5">
                    <Grid item xs={6} md={6}>
                        ...
                        >
                            ...
                            <Select
                                labelId={`demo-simple-select-label-${Price_L_Price}`}
                                id={`demo-simple-select-${Price_L_Price}`}
                                name="name"
                                value={Price_L_Price}
                                onChange={(e) => {
                                    const value = e.target.value;
                                    handleInputChange("Price_L_Price", value);
                                    setPrice_L_Price(value);
                                }}
                                disabled={true}
                            >
