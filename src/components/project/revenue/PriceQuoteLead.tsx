...
import { Box, Button, FormControl, FormControlLabel, Grid, Radio, RadioGroup, Stack } from '@mui/material';
...
interface PriceQuoteLeadProps extends WithFormFieldsStateProps {
...
const PriceQuoteLead: React.FC<PriceQuoteLeadProps> = ({
    onSave,
    formFieldsState,
    setFormFieldsState,
    activeStep,
    ...
    const [PriceQuote_L_PriceQuote, setPriceQuote_L_PriceQuote] = useState<string | null>(null);
    ...
    useMemo(() => {
        ...
        if (formFieldsState.length > 0) {
            ...
            formFieldsState.forEach((fieldState) => {
                if (fieldState.key === 'PriceQuote_L_PriceQuote') {
                    setPriceQuote_L_PriceQuote(fieldState.value);
                }
            }
            );
        }
    ...
    const handleInputChange = (key: string, value: string) => {
        ...
        if (key === 'PriceQuote_L_PriceQuote') {
            setPriceQuote_L_PriceQuote(value);
        }
                ...
                <Grid item xs={4}>
                    <FormControl>
                        ...
                        <RadioGroup
                            ...
                            value={PriceQuote_L_PriceQuote}
                            onChange={(e) => handleInputChange('PriceQuote_L_PriceQuote', e.target.value)}
                        >
                            <FormControlLabel
                                value="Cash Bid"
                                control={<Radio />}
...
export default withFormFieldsState(PriceQuoteLead as React.ComponentType<Omit<PriceQuoteLeadProps, 'formFieldsState' | 'setFormFieldsState' | 'subStepId'>>);