...
const PriceTypeZinc: React.FC<PriceTypeZincProps> = ({
    ...
    const [Price_L_PriceType_Fixed_Select, setPrice_L_PriceType_Fixed_Select] = useState<string | null>(null);
    const [Price_L_PriceType_Fixed_VolType, setPrice_L_PriceType_Fixed_VolType] = useState<string | null>(null);
    ...
    useMemo(() => {
        ...
        if (formFieldsState.length > 0) {
            ...
            formFieldsState.forEach((fieldState) => {
                if (fieldState.key === 'Price_L_PriceType_Fixed_Select') {
                    setPrice_L_PriceType_Fixed_Select(fieldState.value);
                }
                if (fieldState.key === 'Price_L_PriceType_Fixed_VolType') {
                    setPrice_L_PriceType_Fixed_VolType(fieldState.value);
                }
            }
            );
        }
    ...
    const requiredFields = ["Price_L_PriceType_Fixed_Select"];
    ...
    const handleInputChange = (key: string, value: string) => {
        if (key === 'Price_L_PriceType_Fixed_Select') {
            setPrice_L_PriceType_Fixed_Select(value);
        }
        if (key === 'Price_L_PriceType_Fixed_VolType') {
            setPrice_L_PriceType_Fixed_VolType(value);
        }
            ...
            <Grid container spacing={2} xs={8}>
                <Box sx={{ width: '100%' }}>
                    <Box className='mt-[-24px]'>
                        <Tabs value={value} onChange={handleChange} aria-label="PriceTypeLead Table">
                        ...
                        <Grid container spacing={2} className="mb-8">
                            <Grid item xs={4} md={4}>
                                ...
                                <TextField
                                    ...
                                    name="name"
                                    autoComplete="off"
                                    value={Price_L_PriceType_Fixed_Select}
                                    onChange={(e) => {
                                        const value = e.target.value;
                                        handleInputChange("Price_L_PriceType_Fixed_Select", value);
                                    }}
                                    error={validated && !Price_L_PriceType_Fixed_Select}
                                    helperText={
                                        validated &&
                                            (!Price_L_PriceType_Fixed_Select || parseFloat(Price_L_PriceType_Fixed_Select) <= 0)
                            ...
                            <Grid item xs={4} md={4}>
                                <FormControl>
                                    ...
                                    <RadioGroup
                                        ...
                                        // value={Price_L_PriceType_Fixed_VolType}
                                        value={"Other"}
                                        onChange={(e) => handleInputChange('Price_L_PriceType_Fixed_VolType', e.target.value)}
                                    >
                                        <FormControlLabel
                                            value="Historical"
                                            control={<Radio />}
