...
interface RefiningChargeZincProps extends WithFormFieldsStateProps {
...
const RefiningChargeZinc: React.FC<RefiningChargeZincProps> = ({
    onSave,
    formFieldsState,
    setFormFieldsState,
    activeStep,
    ...
    const [RC_Z_Charge, setRC_Z_Charge] = useState<string | null>(null);
    const [RC_Z_Value, setRC_Z_Value] = useState<string | null>(null);
    ...
    useMemo(() => {
        ...
        if (formFieldsState.length > 0) {
            ...
            formFieldsState.forEach((fieldState) => {
                if (fieldState.key === 'RC_Z_Charge') {
                    setRC_Z_Charge(fieldState.value);
                }
                if (fieldState.key === 'RC_Z_Value') {
                    setRC_Z_Value(fieldState.value);
                }
            }
            );
        }
            ...
            <Grid container spacing={2} xs={8}>
                <Grid item xs={3} md={3}>
                    ...
                    >
                        ...
                        <Select
                            labelId={`demo-simple-select-label-${RC_Z_Charge}`}
                            id={`demo-simple-select-${RC_Z_Charge}`}
                            value="1.50"
                            onChange={(e) => handleInputChange('RC_Z_Charge', e.target.value as string)}
                            name="name"
                            disabled={true}
                        >
                            <MenuItem value="1.50">
                ...
                <Grid item xs={3} md={3}>
                    ...
                    >
                        ...
                        <Select
                            labelId={`demo-simple-select-label-${RC_Z_Value}`}
                            id={`demo-simple-select-${RC_Z_Value}`}
                            value="T.oz"
                            onChange={(e) => handleInputChange('RC_Z_Value', e.target.value as string)}
                            name="name"
                            disabled={true}
                        >
                            <MenuItem value="T.oz">
...
export default withFormFieldsState(RefiningChargeZinc as React.ComponentType<Omit<RefiningChargeZincProps, 'formFieldsState' | 'setFormFieldsState' | 'subStepId'>>);