...
interface PenaltiesZincProps extends WithFormFieldsStateProps {
...
const PenaltiesZinc: React.FC<PenaltiesZincProps> = ({
    onSave,
    formFieldsState,
    setFormFieldsState,
    activeStep,
    ...
    const [Penalties_Z_Element, setPenalties_Z_Element] = useState<string | null>(null);
    const [Penalties_Z_Charge, setPenalties_Z_Charge] = useState<string | null>(null);
    const [Penalties_Z_Each, setPenalties_Z_Each] = useState<string | null>(null);
    const [Penalties_Z_Above, setPenalties_Z_Above] = useState<string | null>(null);
    ...
    useMemo(() => {
        ...
        if (formFieldsState.length > 0) {
            ...
            formFieldsState.forEach((fieldState) => {
                if (fieldState.key === 'Penalties_Z_Element') {
                    setPenalties_Z_Element(fieldState.value);
                }
                if (fieldState.key === 'Penalties_Z_Charge') {
                    setPenalties_Z_Charge(fieldState.value);
                }
                if (fieldState.key === 'Penalties_Z_Each') {
                    setPenalties_Z_Each(fieldState.value);
                }
                if (fieldState.key === 'Penalties_Z_Above') {
                    setPenalties_Z_Above(fieldState.value);
                }
            }
            );
        }
    ...
    const handleInputChange = (key: string, value: string) => {
        if (key === 'Penalties_Z_Element') {
            setPenalties_Z_Element(value);
        }
        if (key === 'Penalties_Z_Charge') {
            setPenalties_Z_Charge(value);
        }
        if (key === 'Penalties_Z_Each') {
            setPenalties_Z_Each(value);
        }
        if (key === 'Penalties_Z_Above') {
            setPenalties_Z_Above(value);
        }
                        ...
                        <Grid container spacing={1} xs={8} sx={{marginTop: 1}}>
                            ...
                            <Grid item xs={3} md={3}>
                                ...
                                >
                                    ...
                                    <Select
                                        labelId={`demo-simple-select-label-${Penalties_Z_Each}`}
                                        id={`demo-simple-select-${Penalties_Z_Each}`}
                                        value="%"
                                        name="name"
                                        disabled={true}
                                    >
                            ...
                            <Grid container spacing={1} xs={8} sx={{marginTop: 1}}>
                                ...
                                <Grid item xs={3} md={3}>
                                    ...
                                    >
                                        <h6 className="text-sm pb-2">Unit</h6>
                                        <Select
                                            labelId={`demo-simple-select-label-${Penalties_Z_Above}`}
                                            id={`demo-simple-select-${Penalties_Z_Above}`}
                                            value="%"
                                            name="name"
                                        >
                                            <MenuItem value="%">
...
export default withFormFieldsState(PenaltiesZinc as React.ComponentType<Omit<PenaltiesZincProps, 'formFieldsState' | 'setFormFieldsState' | 'subStepId'>>);